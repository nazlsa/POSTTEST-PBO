...
public class Main {
    ...
    static void hapusKategori() {
        ...
        for (Baju b : listBaju) {
            if (b.getKategori().getIdKategori() == id) {
                System.out.println("  [!] Tidak bisa dihapus, masih dipakai baju: " + b.getNamaBaju());
                return;
            }
        }
    ...
    static void editBaju() {
        ...
        if (!nama.isEmpty())  target.setNamaBaju(nama);
    ...
    static void hapusBaju() {
        ...
        System.out.print("  Yakin hapus '" + target.getNamaBaju() + "'? (y/n): ");
        if (sc.nextLine().trim().equalsIgnoreCase("y")) {
